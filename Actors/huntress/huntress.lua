...
local sprites = {
	...
	shoot1b_1 = Sprite.load("Huntress1Shoot1b_1", "Actors/huntress/shoot1b_1", 16, 7, 9),
	shoot1b_2 = Sprite.load("Huntress1Shoot1b_2", "Actors/huntress/shoot1b_2", 16, 7, 9),
	...
	reticule = Sprite.load("HuntressTarget", "Actors/huntress/reticule", 1, 11, 11),
	bolt = Sprite.load("HuntressBolt2", "Actors/huntress/bolt", 6, 4, 3)
}
...
local reticule = Object.new("HuntressTarget")
reticule.sprite = sprites.reticule
reticule:addCallback("create", function(self)
	local data = self:getData()
	self.alpha = 0
	data.life = 60
end)
reticule:addCallback("step", function(self)
	local data = self:getData()
	self.angle = self.angle + 0.1
	if data.target and data.target:isValid() then
		self.x = data.target.x
		self.y = data.target.y
	end
	if data.life > -1 then
...
reticule:addCallback("draw", function(self)
	local data = self:getData()
	local color = Color.fromRGB(182, 213, 245)
	graphics.drawImage{
		image = sprites.reticule,
		x = self.x,
		y = self.y,
		angle = self.angle,
		color = color
...
local homingBolt = Object.new("HuntressBoltHoming")
homingBolt.sprite = sprites.bolt
homingBolt:addCallback("create", function(self)
	local data = self:getData()
...
homingBolt:addCallback("step", function(self)
	local data = self:getData()
	local this = self:getAccessor()
	self.angle = this.direction
	---------------------------------------
	if data.target and data.target:isValid() then
		data.targetX = data.target.x
		data.targetY = data.target.y
		if self:collidesWith(data.target, self.x, self.y) then
			data.life = -1
		end
	end
	local targetAngle = GetAngleTowards(data.targetX, data.targetY, self.x, self.y)
	this.direction = DirectionStep(this.direction, targetAngle, this.speed)
	---------------------------------------
	data.life = data.life - this.speed
	if data.life <= -1 then
		if data.parent then
			local b = data.parent:fireExplosion(self.x, self.y, 0.25, 1, 1.5, sprites.sparks1, nil)
			b:set("max_hit_number", 1)
			if data.target and data.target:isValid() then
				b:set("specific_target", data.target.id)
			end
			self:destroy()
			return
		end
	...
	---------------------------------------
	if misc.getOption("video.quality") > 1 then
		particles.bolt:angle(this.direction, this.direction, 0, 0, false)
		if data.forceCrit and data.forceCrit == true then
			particles.bolt:burst("below", self.x, self.y, 1, Color.RED)
		else
...
strafe:setEvent("init", function(player, index)
	if initSkill(player, index, player:getAnimation("shoot1"), "strafe2") then
...
local strafeHoming = Skill.new()

strafeHoming.displayName = "Strafe 2.0"
strafeHoming.description = "Fire a seeking arrow for 150% damage. You can shoot all skills while moving. Targets the closest enemy."
strafeHoming.icon = sprites.icons
strafeHoming.iconIndex = 1
strafeHoming.cooldown = 12

strafeHoming:setEvent("init", function(player, index)
	if initSkill(player, index, player:getAnimation("shoot1"), "strafe2") then
		player:setAlarm(index + 1, strafeHoming.cooldown / player:get("attack_speed"))
		return true
	end
	return false
end)
...
local function homingStrafeFire(player)
	if not player:survivorFireHeavenCracker(1) then
		for i = 0, player:get("sp") do
			local xx = player.x + 9 * player:get("upper_half_xscale")
			local yy = player.y - 10 * i
			local bolt = homingBolt:create(xx, yy)
			local data = bolt:getData()
			data.parent = player
			data.damage = player:get("damage")
			data.team = player:get("team")
			local dirDif = math.random(-45, 45)
			bolt:set("direction", (90 - 90 * player:get("upper_half_xscale")) + dirDif)
			data.targetX = xx + (250 * player:get("upper_half_xscale") )
			data.targetY = yy
			local scan = enemies:findLine(xx, yy, xx + (300 * player:get("upper_half_xscale")), yy)
			if scan and scan:isValid() then
				data.target = scan
			end
			data.life = Distance(xx, yy, data.targetX, data.targetY) + math.abs(dirDif)
			local r = reticule:findMatchingOp("parent", "==", player.id)[1]
			if not r then
				r = reticule:create(data.targetX, data.targetY)
				r:set("parent", player.id)
				r:getData().life = data.life / bolt:get("speed")
				if data.target then r:getData().target = data.target end
			else
				r.x = data.targetX
				r.y = data.targetY
				r:getData().life = (data.life / bolt:get("speed"))
...
local flurry = Skill.new()

flurry.displayName = "Flurry"
flurry.description = "Draw back a volley of 3 seeking arrows for 3x100% damage. Critical Strikes fire 6 arrows. Targets the closest enemy."
flurry.icon = sprites.icons
flurry.iconIndex = 1
flurry.cooldown = 12

flurry:setEvent("init", function(player, index)
	local anim = "shoot1b_1"
	player:getData().forceCrit = false
	local crit = math.random(100)
	if crit > player:get("critical_chance") then 
		anim = "shoot1b_2"
		player:getData().forceCrit = true
	end
	if initSkill(player, index, player:getAnimation(anim), "flurry") then
		player:setAlarm(index + 1, flurry.cooldown / player:get("attack_speed"))
		return true
	end
	return false
end)

local function flurryFire(player)
	if not player:survivorFireHeavenCracker(1) then
		for i = 0, player:get("sp") do
			local xx = player.x + 9 * player:get("upper_half_xscale")
			local yy = player.y - 10 * i
			local bolt = homingBolt:create(xx, yy)
			local data = bolt:getData()
			data.forceCrit = player:getData().forceCrit
			data.parent = player
			data.damage = player:get("damage")
			data.team = player:get("team")
			local dirDif = math.random(-45, 45)
		...
		sounds.shoot1:play(1.15 + math.random() * 0.15)
	end
end
...
arrowRainSkill.description = "Teleport into the sky. Target an area using left and right to rain down arrows, slowing all enemies and dealing 110% damage per second."
...
ballista.description = "Teleport backwards into the sky. Fire up to 3 energy bolts, dealing 3x900% damage. Aim using using left and right."
...
registercallback("postStep", function()
	for _, player in ipairs(misc.players) do
		...
		elseif p.strafe2_active == 1 then
			skillStep(player, p, 0.29, 4, homingStrafeFire, "strafe2")
		elseif p.flurry_active == 1 then
			for i = 8, 13 do
				if player:getData().forceCrit and player:getData().forceCrit == true then
					skillStep(player, p, 0.29, i, flurryFire, "flurry")
				else
					if i % 2 == 0 then
						skillStep(player, p, 0.29, i, flurryFire, "flurry")
					end
...
local defaultSprites = {
	...
	["shoot1b_1"] = sprites.shoot1b_1,
	["shoot1b_2"] = sprites.shoot1b_2,
...
local articSprites = {
	...
	["shoot4b_1"] = Sprite.load("HuntressShoot4bSkin1", "Actors/huntress/arctic/shoot4b_1", sprites.shoot4b_1.frames, sprites.shoot4b_1.xorigin, sprites.shoot4b_1.yorigin),
...
loadout:addSkill("Primary", strafeHoming, {
	loadoutDescription = [[Fire a &y&seeking&!& arrow for &y&150%&!& damage.
&b&You can shoot all skills while moving.&!& Targets the closest enemy.]]
})
loadout:addSkill("Primary", flurry, {
	loadoutDescription = [[Draw back a volley of &y&3&!& seeking arrows for &y&3x100% damage&!&. 
Critical Strikes fire 6 arrows. &b&You can shoot all skills while moving.&!& 
...
left and right to &y&rain down arrows&!&, 
...
dealing &y&3x900% damage&!&. Aim using using left and right.]],
locked = true,
})
