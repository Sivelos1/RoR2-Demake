...
relicMissile:addCallback("step", function(self)
    ...
    if data.life > -1 then
        ...
        local nearest = poi:findNearest(self.x, self.y)
        local nearParent = Object.findInstance(nearest:get("parent"))
        if nearParent and nearParent:isValid() then
            if self:collidesWith(nearParent, self.x, self.y) then
                data.life = -1
                return
            end
        end
