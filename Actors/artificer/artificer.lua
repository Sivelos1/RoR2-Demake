...
local sprites = {
	...
	icons = Sprite.load("MageSkills", "Actors/artificer/skills", 9, 0, 0),
...
local ChargeSecondaryInputStep = function(player)
    ...
    elseif m2Phase[player] == 1 then --Startup animation
        if player:get("moveLeft") == 1 then
            player:set("pHspeed", -player:get("pHmax") / 4)
...
local ChargeSecondaryStep = function(player, skill)
    local data = player:getData()
    local p = player:getAccessor()
    ...
    elseif m2Phase[player] == 3 then
        if math.floor(player.subimage) == 3 then
            if p.activity_var1 == 0 then
                ...
                local proj = nil
                if skill == 1 then --nanobomb
                    proj = nanoBomb:create(player.x + (3*player.xscale), player.y)
                elseif skill == 2 then --nanospear

                end
                proj:set("direction", player:getFacingDirection())
                proj:getData().parent = player
                proj:getData().charge = (m2Charge[player] / maxCharge)
                p.activity_var1 = 1
            end
...
launchNanoBomb.iconIndex = 3
launchNanoBomb.cooldown = 3 * 60

local launchNanoSpear = Skill.new()

launchNanoSpear.displayName = "Cast Nano-Spear"
launchNanoSpear.description = "Charge up a piercing nano-spear that deals 400%-1200% damage and freezes all enemies."
launchNanoSpear.icon = sprites.icons
launchNanoSpear.iconIndex = 8
launchNanoSpear.cooldown = 3 * 60
...
icePillar:addCallback("create", function(self)
    local data = self:getData()
    sounds.iceSnd:play(0.9 + math.random() * 0.2)
    ...
    data.state = 0
    data.life = 10*60
    self.y = FindGround(self.x, self.y) - (iceSprites.mask.yorigin)
    data.hit = {}
end)
...
icePillar:addCallback("step", function(self)
    ...
    elseif data.state == 1 then
        ...
        if data.life > -1 then
            ...
            if enemy and enemy:isValid() then
                if not data.hit[enemy] then
                    if self:collidesWith(enemy, self.x, self.y) then
                        ...
                        hit:set("freeze", 1)
                        if enemy:get("hp") <= (enemy:get("maxhp") * 0.3) and not enemy:isBoss() then
                            enemy:kill()
                        end
                        data.hit[enemy] = true
                        data.life = -1
            ...
            data.state = 2
            self.subimage = 1
            sounds.iceDeathSnd:play(0.9 + math.random() * 0.2)
        end
...
snapfreeze.iconIndex = 4
snapfreeze.cooldown = 3 * 60

snapfreeze:setEvent("init", function(player, index) 
    return initActivity(player, index, player:getAnimation("shoot3"), 0.2, true, true)
end)
for i = 4, 7 do
    snapfreeze:setEvent(i, function(player, index) 
        local iceInst = icePillar:create(player.x + ((15 * player.subimage)*player.xscale), player.y)
        iceInst:getData().parent = player
        if math.floor(player.subimage) >= 7 then
...
loadout:addSkill("Utility", snapfreeze,{
    loadoutDescription = [[Create a barrier that &b&freezes&!& enemies for &y&100% damage&!&. 
...
loadout:addSkill("Special", Loadout.PresetSkills.Unfinished)
    ...
    if player:isValid() then
        if player:getSurvivor() == mage then
            ...
            if loadout:getCurrentSkill("Secondary").obj == launchNanoBomb then
                ...
                ChargeSecondaryStep(player, 1)
            end
            if loadout:getCurrentSkill("Secondary").obj == launchNanoSpear then
                ChargeSecondaryInputStep(player)
                ChargeSecondaryStep(player, 2)
            end
